...
<div class="sf-body">
  ...
  <div class="sf-tab-bar" id="sf-main-tab-bar">
    ...
    ${skillLevels.length ? `<button class="sf-tab-btn" data-tab="skills">SKILLS</button>` : ''}
  </div>
  ...
  async function loadSkillCounts() {
    ...
    try {
      const res  = await fetch(APPS_SCRIPT_URL, {
        method: 'POST',
        body:   JSON.stringify({ action: 'getSkillCounts', phone: _auth.phone, schoolName: _auth.schoolName }),
      });
      const data = await res.json();
  ...
  function renderProgressBars(data) {
    ...
    // Academics: one bar per (level, subject) applicable to this school type
    ...
    // Skills: hidden for Primary and Community schools
    let skillsHtml = '';
    if (hasSkills) {
