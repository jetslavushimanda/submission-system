...
const ZoneForm = (() => {
  ...
  let _pageId, _auth, _activeMain, _activeSub;
  ...
  // School select generator
  function buildSchoolSelectHTML(prefix) {
    const opts = zoneSchools()
      .map(s => `<option value="${s.name}">${s.name} (${s.type})</option>`)
      .join('');
    return `
      <div class="field">
        <label for="${prefix}-zf-school">School Participant is Coming From <span class="req">*</span></label>
        <select id="${prefix}-zf-school" class="zf-school-select" data-prefix="${prefix}">
  ...
  <div class="sheet-body">
    ...
    <div id="subtab-learner" class="sf-subtab-panel">
      <div class="form-card">
        ...
        ${buildSchoolSelectHTML('innovations')}
        <div class="field">
        ...
        <!-- Swappable learner fields wrapper -->
...
<div id="sheet-academics" class="slide-sheet">
  ...
  <div class="sheet-body">
    <div class="form-card">
      ...
      ${buildSchoolSelectHTML('academics')}
      <div class="field">
...
<div id="sheet-history" class="slide-sheet">
  <div class="sheet-header">
    <span class="sheet-title">Zone Submissions</span>
...
`;
  ...
  function bindEvents() {
    ...
    // School change selectors
    document.querySelectorAll('.zf-school-select').forEach(sel => {
      sel.addEventListener('change', (e) => {
        const prefix = e.target.dataset.prefix;
        onSchoolChange(prefix);
      });
    });
  ...
  function openSheet(name) {
    if (name === 'history') {
      SubmissionHistory.show('sheet-history-body', _auth, 'zone', ZONE_SLOT_TOTAL);
  ...
  function switchSubTab(subtab) {
    ...
    // Trigger cascades to make sure quota cards update correctly
    onSchoolChange('innovations');
  ...
  function onSchoolChange(prefix) {
    const school      = v(`${prefix}-zf-school`);
    ...
    if (prefix === 'innovations') {
      ...
      if (_activeSub === 'learner') onInnovCatChange();
      else if (_activeSub === 'teacher') onTeacherCatChange();
      else if (_activeSub === 'youth') onYouthCatChange();

    } else if (prefix === 'academics') {
  ...
  function checkSlotUsageAndLimit(cat, lvl, pType) {
    ...
    let max = 3; // Learner Innovations default for Zone Coordinator
    ...
    if (quota.isFull) {
      container.innerHTML = `
        <div class="slot-full-template">
          ...
          <p class="slot-full-desc">The zonal quota limit for category <strong>"${cat}"</strong> at level <strong>"${lvl || 'All'}"</strong> has been reached.</p>
  ...
  function isValid() {
    ...
    if (!v(`${prefix}-zf-school`)) return false;
      ...
      msg.innerHTML = `
        <div class="alert alert-success">
          ...
          <div class="success-actions-row">
            ...
            <button class="btn-success-action btn-add-another" onclick="ZoneForm.resetAndReuseForm('${prefix}')">Add Another</button>
            <button class="btn-success-action btn-goto-history" onclick="ZoneForm.viewSubmissionsFromSuccess('${prefix}')">View My Submissions</button>
          </div>
        </div>`;
    ...
    msg.innerHTML = `
      <div class="alert alert-warning">
        ...
        <div class="warning-actions-row">
          <button class="btn-warning-action btn-bypass-dup" onclick="ZoneForm.submitWithBypass()">Submit Anyway</button>
  ...
  function renderProgressBars(data) {
    ...
    // Zone coordinator gets simple categories since they represent the whole zone
    ['ECE & Primary', 'Junior Secondary', 'Senior Secondary'].forEach(level => {
    ...
    el.innerHTML =
      ['ECE & Primary', 'Junior Secondary', 'Senior Secondary'].map(lvl =>
        innovSection(lvl + ' Innovations', (inn.learner[lvl] || {}), 1)
      ).join('') +
      ...
      `<div class="sf-prog-totals">
        <div class="sf-prog-total-row sf-prog-total-main"><span>Total slots used:</span><span>${data.total || 0} of ${ZONE_SLOT_TOTAL}</span></div>
      </div>`;
  }
  ...
  function collectDraftData() {
    const fields = {};
    const prefix = _activeMain;
    fields[`${prefix}-zf-school`] = v(`${prefix}-zf-school`);
  ...
  function restoreDraft(draft) {
    ...
    if (f[`${prefix}-zf-school`]) {
      setVal(`${prefix}-zf-school`, f[`${prefix}-zf-school`]);
      onSchoolChange(prefix);
    }
  ...
  function resetFormFields() {
    ...
    // Clear school selects
    ['innovations', 'academics', 'skills'].forEach(prefix => {
      const sch = document.getElementById(`${prefix}-zf-school`);
      if (sch) sch.selectedIndex = 0;
      onSchoolChange(prefix);
    });
  ...
  function zoneSchools() {
    return (typeof ZONES !== 'undefined' && ZONES[_auth.zone]) ? ZONES[_auth.zone].schools : [];
  }

  function schoolTypeFor(schoolName) {
    const s = zoneSchools().find(x => x.name === schoolName);
    return s ? s.type : '';
  }
